...
- `skills/figjam-storymap-llm/SKILL.md` — Audit and parse FigJam User Story Maps (Jeff Patton methodology) into LLM-readable Markdown / JSON. Eliminates the post-workshop transcription step. Includes a Python parser (Figma REST API → Markdown / JSON), canonical FigJam template spec, verification prompt for LLM-readiness audits, and Lean UX rules (V1 has priorities + owners + AC; V2 / V3 are clean). Use after a Story Mapping workshop, before publishing a Story Map template, or when feeding a Story Map to a coding agent (Cursor, Claude Code, Copilot).
