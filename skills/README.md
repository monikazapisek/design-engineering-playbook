...
* **[typesetting-engine-skillset](./typesetting-engine-skillset/)** — Six-skill typography/layout family covering the full pipeline from character-level polish to Figma-native design-token generation: `microtypography`, `line-length-optimizer`, `text-typesetting`, `vertical-spacing`, `type-scale-generator`, `glyph-fidelity`. See the skillset's own README for the full map and which skill to use when.
* **[figjam-storymap-llm](./figjam-storymap-llm/)** — Audit and parse FigJam User Story Maps (Jeff Patton methodology) into LLM-readable Markdown / JSON — eliminates the post-workshop transcription step and feeds Story Maps to coding agents (Cursor, Claude Code, Copilot) as living specification. Includes a Python parser (Figma REST API → Markdown/JSON), canonical FigJam template spec, and a verification prompt for LLM-readiness audits. Use after a Story Mapping workshop, before publishing a Story Map template, or when feeding a Story Map to a coding agent.
